function [ L3D, num_objects_in_slice ] = ...
    label_2D_slices( V3D , slice_ind , Dim , min_2D_area)
...
switch Dim
    case 'x'
        [xDim,~,~] = size(V3D);
        num_objects_in_slice = zeros(xDim,1);
    case 'y'
        [~,yDim,~] = size(V3D);
        num_objects_in_slice = zeros(yDim,1);
    case 'z'
        [~,~,zDim] = size(V3D);
        num_objects_in_slice = zeros(zDim,1);
end
