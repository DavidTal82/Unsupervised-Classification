function [ G_X ] = X_separation( Data, L3D_X, x )
...
for ix = min(x):max(x)
    ...
    current_local_slice = squeeze(L3D_X(ix,:,:));
    current_global_slice = squeeze(G_X(ix,:,:));
    
    lin_ind = find(current_local_slice);
    [r,c] = ind2sub([Data.ImageSize(2),Data.ImageSize(3)],lin_ind);
    ...
    [G_X] = update_global_3D_X(G_X,points(points(:,6)==-1,:),ix); 
    
    [ conn_mat, split_flag ] = get_connectiviey( points ,max(G_X(:)));
    ...
    if ~isequal(conn_mat(:,2),conn_mat(:,3)) && split_flag
        ...
        for m = 1:length(g2change)
            G_X = update_previous('x',ix,L3D_X,G_X,conn_mat(g2change(m),:));
    ...
    [ G_X ] = update_global_next_3D_X( G_X, L3D_X, points, ix );
